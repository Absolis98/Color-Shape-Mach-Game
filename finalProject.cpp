...
GLfloat angleCubeY = 0.0f;     // Rotational angle for cube [NEW]
int selectIndex = 1;
int indexSkin = 0;
GLfloat selectPos = 0;
int i, x, y;
...
void drawMainMenu(void) {
	...
	drawRectangle();
	glRotatef(31, 0.0f, 0.0f, 1.0f);
	glTranslatef(1.29f, 0.92f, -2.0f);
	drawTriangle();
	glPopMatrix();
...
void drawDifficulty(void) {
	...
	if (selectedIndex==2){
		glBegin(GL_QUADS);
	glColor4f(0.0f, 0.0f, 1.0f, 0.2f);     // Red
	glVertex3f(1.0f, 1.0f, 0.0f);
	glVertex3f(-1.0f, 1.0f, 0.0f);
	glVertex3f(-1.0f, 0.2f, 0.0f);
	glVertex3f(1.0f, 0.2f, 0.0f);
...
void gameEasyDisplay() {
	//cout << "***COLOR V SIZE: " << colorV.size() << endl;
	
	if (!isSequenceComplete && frames < 65) {
		dispSquare();
	}
	//cout << "displaying easy square" << endl;

	if (frames == 70) {
		//cout << "REACHED FRAME LIMIT" << endl;
		//cout << "Checking if " << colorVIndex + 1 << " is <= " << colorV.size() << endl;
		
		//cout << colorID << " ! " << colorV[0] << endl;
